import { VStack, Text, Button, useDisclosure, Box, Image, Spinner, Flex, } from '@chakra-ui/react'
...
const Profile = () => {
    ...
    return loading ? <Flex h={"100vh"} justifyContent={"center"} align={"center"}>
        <Spinner size={"xl"} color='blueviolet' />
    </Flex> : (
        <VStack justify={"start"} align={"start"} gap={4} w={"100%"}>
            <ProfileCard userData={userData} />
            <Text>{userData?.bio}</Text>
            {
                authUser?._id === userData?._id ? <Button onClick={onOpen} w={"100%"} bg={"crimson"}>Edit Profile</Button> :
                    <Button onClick={() => handleFollowUnfollow(userData)} w={"100%"} bg={"crimson"}>{userData?.followers?.includes(authUser._id) ? "Unfollow" : "Follow"}</Button>
            }
            <Box>
                <ModalComponent
                    flag="isProfile"
