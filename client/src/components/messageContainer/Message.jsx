...
const Message = ({ msg }) => {
    ...
    return (
        <VStack w={"100%"} justify={msg?.senderID === authUser?._id ? "flex-end" : "flex-start"} align={msg?.senderID === authUser._id ? "flex-end" : "flex-start"}>
            <VStack justify={"end"} align={"end"} gap={0} bg={colorMode === "dark" ? msg?.senderID === authUser?._id ? "gray.300" : "gray.900" : msg?.senderID === authUser?._id ? "gray.300" : "gray.200"} px={4} rounded={"xl"}>
                <Text>{msg?.text}</Text>
